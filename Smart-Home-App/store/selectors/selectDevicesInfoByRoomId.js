...
export const selectNonDisplayedDevicesInfoByType = (roomId, deviceType) => createSelector(
    //* input selector
    ...
    (availableRooms, rooms) => {
        ...
        //* filter devices' type
        const selectedAvailableDevicesInfo = filterDevicesByType(selectedAvailableRoom.devices, deviceType)
        const selectedDevicesInfo = filterDevicesByType(selectedRoom.devices, deviceType)

        //* filter devices which are not displayed
        return selectedAvailableDevicesInfo.filter(availableDevice => {
            const isAdded = selectedDevicesInfo.some(device => device.id === availableDevice.id)
            return !isAdded
        })
    }
)